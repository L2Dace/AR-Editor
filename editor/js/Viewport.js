...
function Viewport( editor ) {
	...
	//Small grid
	const grid1 = new THREE.GridHelper( 29, 29, 0x222222  ); ////size, division, color. By setting the size to half the divion gets you 1/4 cuts, which is useful for mesurement
	grid1.material.color.setHex( 0x222222 );
	grid1.material.vertexColors = false;
	grid.add( grid1 );

	//Large grid
	const grid2 = new THREE.GridHelper( 30, 30, 0x222222 ); //size, division, color. By setting the size and division to the same vaule, you get one unit, which is useful for mesurement
	...
	//This needs to be moved to the Menubar.File.js as a flag, and to Editor.JS as a function extention to the Clear funcion
	/*
	////if DefaultMarkerPlaneForScale doesn't exist, add it.
	//console.dir(scene.children);
	//var containsMarker = scene.children.some(function(obj) {
	//	return obj.Name && obj.Name === "DefaultMarkerPlaneForScale";
	//});
	//console.log("containsMarker:");
			//console.dir(containsMarker);
	
	
	if(scene.getObjectByName('DefaultMarkerPlaneForScale')  == undefined) {

		//if(!containsMarker){
			
			//Adding in a plane with the AR Marker on it scaled to one unit.
